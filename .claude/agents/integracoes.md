...
| **Ayrshare** | Distribuição social (7 redes) | `AYRSHARE_API_KEY`, `AYRSHARE_DOMAIN`, `AYRSHARE_PRIVATE_KEY`, `AYRSHARE_WEBHOOK_SECRET` | fail-closed — atrás de `DISTRIBUTION_ENABLED`/`DISTRIBUTION_PUBLISH_ENABLED`, ver [adr-distribution-ayrshare.md](docs/adr-distribution-ayrshare.md) |
...
5. **Collections Firestore como proxy de volume real**: `generatedContents` (quantidade de gerações), `asyncJobs` (fila de jobs — selo/voz/publish), `distributionPosts`/`distributionAuditLog` (Ayrshare), `billingWebhookEvents` (Asaas), `evaluationRuns` (evals). Contar documentos/período nessas collections é o proxy mais direto de "quanto a app está sendo usada" — via Firebase Admin SDK/console, não via UI hoje.
