...
export function OnboardingProvider({ children }: { children: ReactNode }) {
  ...
  const sidebarTarget = useMemo(
    () => (computed.isActive ? resolveSidebarTarget(computed.currentStepId, pathname) : null),
    [computed.isActive, computed.currentStepId, pathname],
  );
  ...
  }, [computed.currentStepId, computed.isActive, isGenerating, mounted]);
  ...
  useEffect(() => {
    if (!mounted || !computed.isActive) {
      return;
    }
    if (computed.currentStepId === "noticias" && pathname.startsWith("/criativo/novo")) {
      markStepDone("noticias");
    }
  }, [mounted, computed.isActive, computed.currentStepId, pathname, markStepDone]);

  const showWelcome = mounted && computed.isActive && !persisted.welcomeSeen;
