...
export function FlowTimeline() {
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-3xl border border-md-border/70 bg-md-bg/70 px-4 py-8 shadow-[0_0_60px_rgba(16,185,129,0.06)] sm:px-8">
        <div className="flex flex-col items-stretch gap-6 md:flex-row md:items-center md:justify-between md:gap-4">
          <div className="min-w-0 flex-1 text-center md:text-left">
            <p className="text-4xl font-bold tabular-nums tracking-tight text-md-text sm:text-5xl">
              {start.time}
            </p>
      ...
      <div>
        <p className="mb-4 text-center text-xs font-semibold uppercase tracking-wider text-md-text-soft">
          Redes atendidas
        </p>
        <ul className="flex flex-wrap items-center justify-center gap-3">
          {networks.map((name) => {
            ...
            return (
              <li key={name}>
                <span
                  className="flex h-12 w-12 items-center justify-center rounded-2xl border border-md-border bg-md-surface/90 shadow-[0_0_20px_rgba(59,130,246,0.12)]"
                  title={name}
                >
                  {SocialIcon ? (
                    <span className={SOCIAL_TILE[name] ?? "text-md-text"}>
                      <SocialIcon size={22} />
                    </span>
                  ) : (
                    <span className="text-[10px] text-md-text-soft">{name}</span>
                  )}
